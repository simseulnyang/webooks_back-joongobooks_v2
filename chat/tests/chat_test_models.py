...
class ChatRoomModelTest(TestCase):
    ...
    def test_unique_together_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            ChatRoom.objects.create(book=self.book, buyer=self.buyer, seller=self.seller)

    def test_different_buyer_can_create_chatroom(self):
        """다른 구매자는 같은 책에 대해 채팅방 생성 가능"""
        another_buyer = UserFactory()
        chatroom2 = ChatRoomFactory(book=self.book, buyer=another_buyer, seller=self.seller)
...
class MessageModelTest(TestCase):
    def setUp(self):
        ...
        self.message = MessageFactory(chatroom=self.chatroom, sender=self.sender)
